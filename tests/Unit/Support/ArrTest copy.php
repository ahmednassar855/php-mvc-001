...
{
    //At the Array Test first function , it doesn't work due to the arrangement of the extracted array in assert-same ( it shall be the same arrange for what we need to check, so it work for [ahmed] then [php]
    public function test_it_returns_only_the_keys_given_from_array()
    {
        $array = [
                    'ahmed' => ['age' => 19],
                    'mohamed' => ['age' => 29],
        ...
        $extracted = Arr::only($array ,['node','ahmed']);
    ...
    public function test_it_checks_for_accessiblility_in_array_items()
        {
            $data = [
                'username' => 'ahmed.nassar',
                'emails' => [
                    'primary' => 'ahmed@gmail.com',
                    'secondary' => 'ahmed.nassar@gmail.com'
                    ]
                ];
            $this -> assertFalse(Arr::accessible('string'));

            $this -> assertTrue(Arr::accessible($data['emails']));
            $this -> assertFalse(Arr::accessible($data['username']));
        ...
        public function test_it_modifies_an_item_with_its_key_or_index(){
            ...
            // (2nd case)     if not found the key add new key with its new value
            Arr::set($data, 0 ,'mahmoud');  // arr set to add new array $data with its key 0 and its value mahmoud
            $this -> assertEquals('mahmoud',$data[0]);

            Arr::set($data, 'emails.primary' , 'anotheremail@example.com');
            $this -> assertEquals('anotheremail@example.com', $data['emails']['primary']);

            Arr::set($data ,'emails.optional.third', 'abc');
            $this -> assertEquals('abc',$data['emails']['optional']['third']);
        }

        public function test_it_sets_the_whole_array_to_be_the_value_if_key_is_not_given(){
            $array = [1, 2, 3, 4] ;
            
            // if not have a key set item equal the value --- (1st case)
        ...
        // 3ayzeen ne3ml add le item mo3ayana fe makan mo3ayan

        public function test_it_adds_item_to_an_array(){
            //$numbers = [1,2,3,4];
            
            //$newArray = Arr::add($numbers, 1, 5); // han3ml add fe index no 1 ne3ml add be value 5 and make shift for all array el gededa hatkoon [1,5,2,3,4]
            $names = ['ahmed' , 'mahmoud' ,' amr'];

            $newArray = Arr::add($names, 1, 'ismail');
            $this -> assertEquals('ismail',$newArray[1]);
        }
